...
# ななはぴアーカイブ自動更新スクリプト
# make allを実行し、変更されたアーカイブファイルをGitでコミット&プッシュします
...
# ログファイルの設定
LOG_FILE="/tmp/nanahapi-update-$(date +%Y%m%d-%H%M%S).log"
exec > >(tee -a "$LOG_FILE")
exec 2>&1

echo "🌸 ななはぴアーカイブ自動更新開始: $(date)"
...
COMMIT_MSG="🤖 自動更新: アーカイブデータ更新 ($COMMIT_DATE)
